...
void my_connect_callback_publish(struct mosquitto *msq, void *obj, int result)
{
	...
	if(!result){
		switch(mode){
			...
			case MSGMODE_STDIN_FILE:
				rc = mosquitto_publish(msq, &mid_sent, topic, msglen, message, qos, retain);
				printf("Publish\r\n");
				break;
			case MSGMODE_NULL:
				rc = mosquitto_publish(msq, &mid_sent, topic, 0, NULL, qos, retain);
				break;
...
void my_disconnect_callback(struct mosquitto *msq, void *obj, int rc)
{
	connected = false;
}

void my_publish_callback(struct mosquitto *msq, void *obj, int mid)
{
	last_mid_sent = mid;
...
void my_log_callback_publish(struct mosquitto *msq, void *obj, int level, const char *str)
{
	printf("%s\n", str);
}
...
{
	printf("[1]\r\n");
	struct mosq_config Config;
	struct mosquitto *msq = NULL;
	int rc;
	int rc2;
	char *buf;
	int buf_len = 1024;
	...
	printf("[3]\r\n");
	memset(&Config, 0, sizeof(struct mosq_config));
	/*rc = client_config_load(&Config, CLIENT_PUB, argc, argv);
	if(rc){
		client_config_cleanup(&Config);
		if(rc == 2){
			/* --help */
/*			print_usage();
		}else{
			fprintf(stderr, "\nUse 'mosuitto_pub --help' to see usage.\n");
		}
		return 1;
	}*/
	printf("[4]\r\n");
    init_config(&Config);
    Config.host = strdup(ipaddress);
    Config.topic = strdup(topicname);
    Config.msglen  = msgconlen;
    Config.message = strdup(messagecontent);
	Config.pub_mode = MSGMODE_CMD;
    
    printf("[5]\r\n");
	topic = Config.topic;
	message = Config.message;
	msglen = Config.msglen;
	qos = Config.qos;
	retain = Config.retain;
	mode = Config.pub_mode;
	username = Config.username;
	password = Config.password;
	quiet = Config.quiet;

	if(Config.pub_mode == MSGMODE_STDIN_FILE){
		if(load_stdin()){
			fprintf(stderr, "Error loading input from stdin.\n");
			return 1;
		}
	}else if(Config.file_input){
		if(load_file(Config.file_input)){
	...
	if(client_id_generate(&Config, "mosqpub")){
		return 1;
	}

	msq = mosquitto_new(Config.id, true, NULL);
	if(!msq){
		switch(errno){
			case ENOMEM:
				if(!quiet) fprintf(stderr, "Error: Out of memory.\n");
	...
	printf("[7]\r\n");
	if(Config.debug){
		mosquitto_log_callback_set(msq, my_log_callback_publish);
	}
	mosquitto_connect_callback_set(msq, my_connect_callback_publish);
	mosquitto_disconnect_callback_set(msq, my_disconnect_callback);
	mosquitto_publish_callback_set(msq, my_publish_callback);
	printf("[8]\r\n");
	if(client_opts_set(msq, &Config)){
		return 1;
	}
	printf("[8.1]\r\n");
	rc = client_connect(msq, &Config);
	if(rc) return rc;
	printf("[8.2]\r\n");
	if(mode == MSGMODE_STDIN_LINE){
			...
			if(status == STATUS_CONNACK_RECVD){
				...
				while(fgets(&buf[pos], read_len, stdin)){
					...
					if(buf[buf_len_actual-1] == '\n'){
						...
						rc2 = mosquitto_publish(msq, &mid_sent, topic, buf_len_actual, buf, qos, retain);
						if(rc2){
